...
namespace gamtetyper.code
{
	public class z_testing
	{
		...
		public static class ZLib
		{
			...
			public static byte[] BufferFromStream(MemoryStream ms, int offset = TypeExtensions.kNoneInt32, int length = TypeExtensions.kNoneInt32, bool skipHeader = true)
			{
			...
			*/
			...
			public static byte[] LowLevelCompress(byte[] bytes, int level, out uint adler, byte[] compressedBytes, bool trimCompressedBytes = true, bool noZlibHeaderOrFooter = true)
			{
				int compressed_size;
			...
			public static uint LowLevelDecompress(byte[] compressedBytes, byte[] uncompressedBytes, bool noHeader = true)
			{
			...
			public static byte[] LowLevelCompress(byte[] bytes)//, Shell.EndianFormat byteOrder)
			{
				...
				var zip = new ICSharpCode.SharpZipLib.Zip.Compression.Deflater(ICSharpCode.SharpZipLib.Zip.Compression.Deflater.BEST_COMPRESSION, false);
				{
					zip.SetInput(bytes);
					zip.Finish();
